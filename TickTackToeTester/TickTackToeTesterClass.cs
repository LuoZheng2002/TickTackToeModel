...
using TickTackToe;
using TickTackToeTester.Properties;
namespace TickTackToeTester
{
	public class TickTackToeTesterClass: TesterBase
	{
		public TickTackToeModel Model { get; } = new TickTackToeModel();
		...
		private void DrawBoard()
		{
			...
			for(int i = 0;i < 3;i++)
			{
				for(int j = 0;j < 3;j++)
				{
					...
					{
						DrawSprite(Resources.black.ToImage<Bgr, byte>(), 108*j + 4, 108*i + 4);
					}
					else if (Model.Chessboard[i,j] == Stone.White)
					{
						DrawSprite(Resources.white.ToImage<Bgr, byte>(), 108 * j + 4, 108 * i + 4);
					}
				}
			}
		}
		...
		public override void OnLeftButtonDown(double x, double y)
		{
			...
			TickTackToeMove move = new TickTackToeMove(Model.WhoseTurn, realX, realY);
			...
			if (validMove)
			{
				...
				try
				{
					...
					{
						SendMessage($"AI落子成功");
						TickTackToeMove aiMove = (TickTackToeMove)info.Move!;
